...
class Application(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        self.dragging = False
        # DROP
        # da canvas a frame o non funziona
    ...
    def populate_canvas(self):
        ...
        self.bin_image = tk.PhotoImage(file="trash.png")
    ...
    def arrange_item(self, lpad, tpad, wpadx, wpady, ww, wh, r, rr, rtext):
        ...
        # x column - y row
        x2r = x1 + ww
        y2r = y1 + wh + rect_height_text#self.linespace*2
...
########## ITEM CREATION
    ...
    # riturn the max text width for each row
    ...
    # the text - return the height of each item text
...
## DRAG
    ...
    def drag_init(self, event):
        ...
        if self.dragged_items:
            ...
            self.dragging = True
            #return ((ASK, COPY, MOVE), (DND_FILES, DND_TEXT), data)
            return ((COPY, COPY), DND_FILES, data)
...
## DROP
    ...
    def drop(self, event):
        ...
        # drag and drop in this same program
        if self.dragging:
            ...
            # dest coords
            print("*drop2::", self.xpos, self.ypos)
            # item dragged
            ...
            if len(selected_item) == 1:
                if selected_item[0] == 1:
                    ...
                    if len(self.dragged_items) == 1:
                        ...
                        num_rect = len(self.d)
                        # number of rect per row 
                        num_rect_row = int(self.dim_w/(wpadx*2+ww+lpad+rpad)) or 1
                        # number of lines max
                        ...
                        if pos_x > num_rect_row or pos_y > num_rrow or pos_x < 0 or pos_y <0:
                            return 'break'
                        ...
                        if item and len(item) > 1:
                            print("found item::", self.w.gettags(item[1]))
                            return 'break'
                ...
                if drag_item == dest_item:
                    ...
                    if os.path.isdir(self.working_dir+"/"+item_tag[1]):
                        ...
                        if event.data:
                            ...
                            action = event.action
                            print("action::", action)
                            print("copying in the dir {}".format(item_tag[1]))
        ...
        # drop from another program
            ...
            if event.data:
                ...
                action = event.action
                print("action::", action)
                ## find the coordinates in the program
...
######## MOUSE BINDINGS
    ...
    def mouseUp(self, event):
        ...
        if self.drag_mouse:
            ...
            for item in self.dragged_items:
                print("Selected:", self.w.gettags(item)[1])
    ...
    def mouseDoubleDown(self, event):
        ...
        if len(selected_item[0]) > 1:
            selected_item_id = selected_item[0][1]
            item_tag = self.w.gettags(selected_item_id)
            print("Double click on: ", item_tag[1])
    ...
    def mouseRDown(self, event):
        ...
        if len(selected_item[0]) > 1:
            ...
            # if it is selected with others
                ...
                for item in self.dragged_items:
                    print("RMB - Selected:", self.w.gettags(item)[1])
        ...
        if len(selected_item[0]) > 1:
            ...
            if selected_item_id in self.rectangle_list:
                ...
                # deselect it if it is selected
                    ...
                    self.w.itemconfigure(selected_item_id, fill="")
