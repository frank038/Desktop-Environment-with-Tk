...
class getMenu():
    ...
    def __init__(self):
        ...
        self.locale_lang = "en"
        try:
            self.locale_lang = locale.getlocale()[0].split("_")[0] # ottengo it
        except:
            self.locale_lang = "en"
        print("locale_lang::", self.locale_lang)
        ...
        ######****######
        #print("self.info_desktop::", self.info_desktop)
        ...
        for ffile in self.file_list:
            ...
            try:
                ...
                if ftype == "Application" and fcategory != "":
                    ...
                    dentry.path = ffile
                    #print("DD::", dentry.name, dentry.categories)
    ...
    def fpop_lists(self):
        ...
        for item in  self.info_desktop:
            ...
            if ret:
                ...
                #list_one.append([item.name, pexec, idx, item.name.lower(), icon])
                # the id of the category
                idx_cat = self.id_list.index(ret)
                idx = self.id_list[idx_cat]
                pexec = DesktopEntry.DesktopEntry(item.path).getExec()
                icon = DesktopEntry.DesktopEntry(item.path).getIcon()
                #print("RR::", item.name, item.path, idx_cat, self.id_list)
                ...
                if ret:
                    ...
                    pexec = DesktopEntry.DesktopEntry(item.path).getExec()
                    icon = DesktopEntry.DesktopEntry(item.path).getIcon()
                    #print("RR2::", item.name, item.path, idx)
                    list_one.append([item.name, item.path, idx, item.name.lower(), pexec, icon])
                else:
                    # Missed categoty
                    #idx_cat = self.id_list.index("Missed")
                    idx = "Missed"#self.id_list[idx_cat-1]
                    pexec = DesktopEntry.DesktopEntry(item.path).getExec()
                    icon = DesktopEntry.DesktopEntry(item.path).getIcon()
                    #print("RR3::", item.name, item.path, idx)
                ...
                #ret = self.item_in_main2(item)
                    ...
                    #ret = self.item_in_ext(item)
                        ...
                        ### icon
                        ##icon = DesktopEntry.DesktopEntry(item.path).getIcon()
                        ##list_two.append([item.name, pexec, idx, item.name.lower(), icon])
                        ## Missed categoty
                        #idx_cat = self.id_list.index("Missed")
                        #idx = self.id_list[idx_cat-1]
        ...
        # do not work
        #list_one2 = sorted(list_one, key=lambda list_one: (list_one[3], list_one[2]), reverse=False)
        # sorted by category
        ...
        #list_two2 = sorted(list_two, key=lambda list_two: (list_one[0], list_one[1]), reverse=False)
        
        ##for ii in list_two2:
            ##pass
            ##self.tv.insert(ii[2], 0, text=ii[0], tags=(ii[1],"E"))
        #
        #print("list_one2::", list_one2)
        #print("list_two2::", list_two2)
...
## escludo la categoria Missed
#for el in mn[0]:
    #print(el)
    ##ff = el[1]
    ##entry = DesktopEntry.DesktopEntry(ff)
    ##print(entry.getExec())
    ##print(entry.getTryExec() or "NN")
    #print("\n")

## to get the path of the icona
## only if it not have full path only png of the hicolor theme
