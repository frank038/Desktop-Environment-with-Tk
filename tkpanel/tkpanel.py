...
rootx.change_attributes(event_mask=Xlib.X.PropertyChangeMask|Xlib.X.SubstructureNotifyMask|Xlib.X.StructureNotifyMask)
...
### da sostituire con soluzioni Xlib
            ...
            while True:
                ...
                def handle_event(event):
                    ...
                    if (event.type == Xlib.X.PropertyNotify):
                        ...
                        ## ma si verifica anche quando cambia la finestra attiva
                                ...
                                if old_list_normal_win != list_normal_win:
                                    ...
                                    for el in list_normal_win:
                                        if el not in old_list_normal_win:
                                            # se elemento non in active_virtual_desktop vuol dire che ha cambiato desktop virtuale
...
class Application(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        ####### THE SYSTRAY
        self.systray_frame = ttk.Frame(self, width=100)
        self.systray_frame.grid(row=0, column=7, sticky="ns", padx=2)
        self.systray_frame_id = self.systray_frame.winfo_id()
        self.master.update_idletasks()
        try:
            prog = "python3 user_modules/xsystray.py {} &".format(self.systray_frame_id)
            ret = subprocess.run([prog], shell=True)
        except:
            print("Some problems with the systray.")
