...
for winid in winid_list:
    ...
    if prop:
        if prop.value.tolist()[0] == NET_CLIENT_LIST_ATOM_N:
            ...
            # workaround for the char "—" chr(8212) recognized as bytes
            if not isinstance(window.get_wm_name(), str):
                window_name = "Unknown"
            else:
                window_name = window.get_wm_name()
            #list_normal_win.append((right_hex(hex(window.id)), window.get_wm_name(), on_desktop))
...
class Application(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        # needed to shorten the buttons when all them fit the frame
        for i in range(20):
